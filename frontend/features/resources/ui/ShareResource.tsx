...
export const ShareResource = () => {
	...
	const saveResource = async () => {
		...
		if (typeof resourceType !== 'string' || !Object.values(RESOURCE_TYPES).includes(resourceType)) {
			setErrors((prevErrors) => [...prevErrors, "Valid resource type is required."]);
			return;
		}
		try {
			...
			await createResource(resource).then(() => {
				clearForm();
			});
		} catch (error) {
			console.error("Error creating resource:", error);
			setErrors((prevErrors) => [...prevErrors, "Failed to create resource. Please try again."]);
		}
	};
